<div class="container">
    ...
    <div class="row">
        <?php foreach ($mangaList as $manga): ?>
            <div class="col-md-3">
                <div class="card mb-4">
                    <img src="<?php echo htmlspecialchars($manga['image_url']); ?>" class="card-img-top"
                         alt="<?php echo htmlspecialchars($manga['name']); ?>" height="400px">
                    <div class="card-body">
                        <h5 class="card-title"><?php echo htmlspecialchars($manga['name']); ?></h5>
                        <?php if (!empty($manga['release_date'])): ?>
                            <p class="card-text">Release
                                Date: <?php echo htmlspecialchars($manga['release_date']); ?></p>
                        <?php endif; ?>
                        <?php if ($manga['average_rating'] > 0): ?>
                            <p class="card-text">
                                <strong>Rating:</strong> <?php echo htmlspecialchars($manga['average_rating']) ?></p>
                        <?php endif; ?>
                        <a href="/manga/<?php echo $manga['id']; ?>" class="btn btn-outline-primary w-100">View
                            Details</a>
                    </div>
                </div>
            </div>
